...
function ImageEncryptAES
close all;
tic
 [i11,i12]=uigetfile('*.*');%XXXXXX Input Image XXXXXXXXXXX
 i12=strcat(i12,i11);
 im2=imread(i12);
%im2=imread('Jellyfish.jpg');
% im12=imread('1.jpg');
% initilize IWD parameter
tic;
N_IWD=5;
N_C=100;
a_v=1000;
...
% check probabilityfor all pixel in water drop
for nc=1 : N_C
    for k=1 : N_IWD
...
% total computational complexity for key length with IWD
    for k=1 : N_IWD
        display(k);
...
% end
% figure(2);
% plot all function /results
figure;
%subplot(2,2,2);
plot(z1,z2,'r<');
hold on;
plot(z1(cell2mat(v_c(v2,1))),z2(cell2mat(v_c(v2,1))));
title('Encrypted form in moving finite intelligence water drop');
figure;
%subplot(2,2,3);
imshow(uint8(a1),[]);
...
figure;
imhist(uint8(im3));
title('Histogram of Selectively finite intelligence Encrypted image');
%subplot(233)
figure;
imhist(uint8(im5));
...
tco=[base  baseAES; proposed   AESProposed];
bar(tco);
legend('AES','IWD-AES');
title('Time Performance for(1st Encryption /2nd Decryption)');


figure
title('Trust Factor for finite computing');
t = 0:b_s:N_C;
x = radius*t;
y = -9.8*t.^2/2;
comet(x,y,0.1)
